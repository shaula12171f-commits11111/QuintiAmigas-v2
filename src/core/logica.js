...
// Sinónimos de genitalia masculina
const SINONIMOS_VERGA = /\b(polla|pija|poronga|pichula|pito|rabo|pene|verga)\b/gi;

/** Normaliza sinónimos (polla/pija/poronga/pichula/pito → verga) para que los patrones coincidan. */
function normalizarSinonimosSexuales(texto) {
  ...
  // También añadir "pene" para tags que usan esa palabra
...
function esSoloMuestra(mensaje) {
  ...
  const muestra = /\b(le |te |me )?muestro (mi )?(pija|verga|polla|pene|poronga|pichula)|saco (la )?(pija|verga|polla|pene|poronga|pichula)|mir[aeá] (mi )?(pija|verga|polla|pene|poronga|pichula)|ve(s|an)? (mi )?(pija|verga|polla|pene|poronga|pichula)|te dejo ver|para que (la )?veas|mirá (esto|mi)\b/i.test(m);
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  if (!pideOral) {
    ...
    elegido = normalizarTag(chica, elegido, false);
  }
