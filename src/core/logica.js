...
function construirContexto(mensajeUsuarioActual = '') {
  ...
  if (mensajeUsuarioActual && esSoloMuestra(mensajeUsuarioActual) && estado.fase !== FASE.INTIMO) {
    lineas.push(
      ...
      'imagen_tag OBLIGATORIO: Viendo_Verga (o hablando / reacción). NUNCA chupando_*.'
    );
  }
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  if (soloMuestra) {
    ...
    if (tagInferido && !/chup|mamad|oral/i.test(tagInferido)) {
      ...
      elegido = 'Viendo_Verga';
      razon = 'usuario solo muestra → FORZADO Viendo_Verga';
    }
  } else if (hayAccion) {
  ...
  // Doble seguridad: si era soloMuestra y el normalizar devolvió algo oral, forzar de nuevo
  if (soloMuestra && /chup|mamad|oral/i.test(elegido)) {
    elegido = normalizarTag(chica, 'Viendo_Verga', true);
    razon += ' + re-forzado Viendo_Verga';
  }
...
export async function enviarMensaje(mensajeUsuario) {
  ...
  if (soloMuestraUsuario) {
    system +=
      ...
      'imagen_tag OBLIGATORIO = Viendo_Verga (o hablando). NUNCA uses chupando_*. ' +
