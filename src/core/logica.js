...
export function iniciarHistoria(chica, historiaId) {
  ...
  const welcomeSexual = /chup|mamad|pija|verga|foll|coño|boxers|sábanas.*chup|te saca la|en la boca/i.test(texto);
  if (welcomeSexual) estado.fase = FASE.INTIMO;
  const soloNoSex = !welcomeSexual && estado.fase !== FASE.INTIMO && !PATRON_SEXO.test(texto);
  let tagInferido = inferirTagFuerte(chica, texto, '', soloNoSex);
  // Confesión y otras historias soft → SIEMPRE hablando (o besando si hay beso)
  const softIds = /confesion|cita|hotel|biblioteca/i;
  if (softIds.test(String(historiaId || '')) || soloNoSex) {
    if (/beso|besarte|besando/i.test(texto)) tagInferido = normalizarTag(chica, 'besando', true) || 'hablando';
    else tagInferido = 'hablando';
  }
  if (soloNoSex && /muestra|verga|viendo_verga/i.test(String(tagInferido || ''))) tagInferido = 'hablando';
  // Forzar explícito para confesión de Nino
  if (String(historiaId || '').includes('confesion')) tagInferido = 'hablando';
  const media = resolverImagen(chica, tagInferido, true);
  if (media.descripcion) estado.outfitActual = { chica, tag: media.tag, descripcion: media.descripcion };
  else estado.outfitActual = null;
...
function extractAccionParaChica(mensaje, chica) {
  ...
  for (const k of keys) {
    for (const { re, accion } of posePatterns) {
      const cerca = new RegExp(`(?:${re.source}).{0,40}\\b${k}\\b|\\b${k}\\b.{0,40}(?:${re.source})`, 'i');
      if (cerca.test(raw) || (named && re.test(t))) return accion;
    }
  }
  if (named || grupal) {
    for (const { re, accion } of posePatterns) {
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  if (soloNoSex) {
    if (esTagSex(elegido) || /muestra.*verga|viendo_verga|ve_mi_verga|chup|mamad|foll|doggy|cowgirl|anal|oral|pene|handjob|paja/i.test(String(elegido || ''))) {
      elegido = normalizarTag(chica, 'hablando', true) || 'hablando';
      razon = (razon || '') + '+forzado_nosex';
    }
  }
...
export async function enviarMensaje(mensajeUsuario) {
  ...
  logGroup('Decision', { targets, fase: estado.fase, escenaSex, soloNoSex, mensajeUsuario });
  const parsed = await enviarMensajeSingle(mensajeUsuario, soloNoSex, escenaSex, dirigidas);
  postProcesarFase(parsed.respuesta);
  const textoCompleto = parsed.respuesta;
  let bloques = partirBloquesMulti(parsed.respuesta, estado.chica).map((b) => ({ ...b, imagen_tag_modelo: parsed.imagen_tag || '' }));
  ...
  const partes = bloques.map((b) => {
    ...
    const { elegido, razon, tagDinamico } = elegirTag(b.chica, b.imagen_tag_modelo || '', b.texto, mensajeParaTag, ahoraSoloNoSex);
    ...
    if (ahoraSoloNoSex && (esTagSex(tagFinalElegido) || /muestra|verga|chup|foll|doggy/i.test(String(tagFinalElegido || '')))) {
      tagFinalElegido = 'hablando';
    }
