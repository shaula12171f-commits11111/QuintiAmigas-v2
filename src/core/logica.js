...
// Solo acciones sexuales reales. Nombrar "verga/pija" o "culo" NO cuenta como escena sexual.
// Incluye sinónimos de oral: chupar, mamar, lamer, etc.
...
// Verbos orales claros (para forzar tag oral)
...
// Helper: usuario solo muestra la verga sin pedir acto
function esSoloMuestra(mensaje) {
  const m = String(mensaje || '').toLowerCase();
  const muestra = /\b(le |te |me )?muestro (mi )?(pija|verga|polla)|saco (la )?(pija|verga|polla)|mir[aeá] (mi )?(pija|verga|polla)|ve(s|an)? (mi )?(pija|verga|polla)|te dejo ver|para que (la )?veas|mirá (esto|mi)\b/i.test(m);
...
function actualizarFaseSegunUsuario(mensaje) {
  ...
  // Solo subir a íntimo si pide ACTO, no solo "mostrar pija"
...
function construirContexto(mensajeUsuarioActual = '') {
  ...
  if (estado.chicasActivas.length > 1) {
    lineas.push(
      'MULTI: Hay varios personajes. Cada uno puede hablar en bloques [Nombre]: texto. Deben reaccionar entre ellos y a lo último que se dijo.'
    );
  }
  ...
  if (estado.fase === FASE.INTIMO) {
    lineas.push('Escena íntima activa: sé explícita, sucia y en el momento. imagen_tag = la acción que ESTÁS haciendo ahora.');
  } else {
    lineas.push('Escena SIN acto sexual pedido todavía. Usá tags no-sex o de reacción.');
  }

  // Refuerzo extra cuando el usuario solo muestra
  if (mensajeUsuarioActual && esSoloMuestra(mensajeUsuarioActual) && estado.fase !== FASE.INTIMO) {
    lineas.push(
      '⚠️ REGLA CRÍTICA DE ESTE TURNO: El usuario SOLO está mostrando / sacando su verga. ' +
      'Reaccioná de forma realista (sorpresa, excitación, mirás fijamente, comentás, te sonrojás, te acercás, te mordés el labio). ' +
      'PROHIBIDO ABSOLUTO: chupar, meter en la boca, lamer la punta, tomar la pija, empezar oral o cualquier sexo. ' +
      'Solo reacción + diálogo caliente. Esperá a que el usuario pida explícitamente que chupes o que continúe. ' +
      'imagen_tag OBLIGATORIO: usuario_muestra_su_verga (o hablando / reacción). NUNCA chupando_*.'
    );
  }
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  // Inferir SIN restricción soloNoSex primero, para poder corregir después
  ...
  // Nivel oral SOLO desde el mensaje del USUARIO (nunca del bot)
  ...
  const soloMuestra = esSoloMuestra(user);

  if (soloMuestra) {
    // PRIORIDAD ABSOLUTA: usuario solo muestra → NUNCA tag de chupar
    if (tagInferido && !/chup|mamad|oral|lam/i.test(tagInferido)) {
      elegido = tagInferido;
      razon = 'usuario solo muestra → tag mirar/ver (inferido)';
    } else if (tagModeloNorm && !/chup|mamad|oral|lam/i.test(tagModeloNorm)) {
      elegido = tagModeloNorm;
      razon = 'usuario solo muestra → modelo no-oral';
    } else {
      elegido = 'usuario_muestra_su_verga';
      razon = 'usuario solo muestra → FORZADO usuario_muestra_su_verga';
    }
  } else if (pideOral) {
    // PRIORIDAD ALTA: usuario pidió oral explícitamente → forzar tag oral
    // Preferir inferencia libre (sin soloNoSex) porque el usuario ya pidió el acto
    if (pidePunta) {
      elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_solo_la_punta_del_pene', false);
      razon = 'usuario pidió oral (punta) → forzado oral';
    } else if (pideMitad) {
      elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_solo_la_mitad_del_pene', false);
      razon = 'usuario pidió oral (mitad) → forzado oral';
    } else if (pideTodo) {
      elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_todo_el_pene', false);
      razon = 'usuario pidió oral (todo) → forzado oral';
    } else {
      // Oral genérico: preferir lamiendo_pene o chupando si existen
      ...
      for (const c of candidatos) {
        ...
        if (n && n !== 'hablando' && /chup|lam|oral|pene|verga/i.test(n)) {
          elegido = n;
          break;
        }
      }
      razon = 'usuario pidió oral → forzado tag oral';
    }
  } else if (hayAccion) {
    ...
    razon = 'sin acción clara';
  }

  // Normalizar final respetando soloNoSex SOLO si no es pedido oral del usuario
  if (!pideOral) {
    elegido = normalizarTag(chica, elegido, soloNoSex);
  } else {
    elegido = normalizarTag(chica, elegido, false); // usuario pidió oral → permitir tags sex
  }

  // Doble seguridad: si era soloMuestra y quedó oral o hablando, forzar tag real de mostrar
  if (soloMuestra && (/chup|mamad|oral|lam/i.test(elegido) || elegido === 'hablando')) {
    ...
    if (forzado && forzado !== 'hablando') {
      elegido = forzado;
      razon += ' + re-forzado usuario_muestra_su_verga';
    }
  }

  // Corrección final: si usuario pidió oral y el tag quedó no-oral, forzar uno oral
  if (pideOral && !/chup|lam|oral|pene|verga|mam/i.test(elegido)) {
    const forzadoOral = normalizarTag(chica, 'lamiendo_pene', false) ||
                        normalizarTag(chica, 'chupando_solo_la_punta_del_pene', false) ||
                        normalizarTag(chica, 'chupando_todo_el_pene', false);
    if (forzadoOral && forzadoOral !== 'hablando') {
      elegido = forzadoOral;
      razon += ' + CORRECCIÓN FINAL oral';
    }
  }
...
export async function enviarMensaje(mensajeUsuario) {
  ...
  // Escena sexual solo si el USUARIO pidió acto real (no por nombrar "verga")
  ...
  if (estado.chicasActivas.length > 1) {
    const extras = estado.chicasActivas
      ...
      .join('\n\n');
    system += `\n\nOTROS PERSONAJES PRESENTES (pueden hablar en bloques [Nombre]: ):\n${extras}\n\nFormato [Nombre]: ... imagen_tag de la chica PRINCIPAL (${estado.chica}).`;
  }

  // Refuerzo ultra-fuerte si solo muestra
  if (soloMuestraUsuario) {
    system +=
      '\n\n⚠️⚠️ REGLA DE ORO ESTE TURNO: El usuario SOLO mostró/sacó la pija. ' +
      'Reaccioná (mirás, te calentás, comentás, te acercás, te pones colorada). ' +
      'PROHIBIDO: chupar, lamer, meter en boca, empezar oral o sexo. ' +
      'imagen_tag OBLIGATORIO = usuario_muestra_su_verga (o hablando). NUNCA uses chupando_*. ' +
      'Esperá a que el usuario diga explícitamente "chúpamela", "métetela en la boca", etc.';
  }
  ...
  logGroup('Raw API', { rawLength: (raw || '').length, parseOk: !!parsed, preview: (raw || '').slice(0, 250) });
  ...
  if (!parsed) {
    for (const extra of PROMPTS_REINTENTO) {
      raw = await llamarGroq([
        ...
        { role: 'user', content: 'Corrige SOLO JSON. Si solo mostró la pija, no uses chupando_*.' }
      ]);
  ...
  // No subir a INTIMO solo porque el bot inventó oral; sí si el usuario pidió acto real
  ...
  // Si el usuario solo mostró, forzar tags no-sex aunque el bot haya inventado oral
  // Si el usuario pidió oral/sexo explícito (incluyendo "lame"), abrir tags sexuales
  ...
  const partes = bloques.map((b) => {
    if (b.chica === 'Aldo') {
      return { chica: 'Aldo', texto: b.texto, imagenUrl: '', audioUrl: '', descripcionImg: '', imagen_tag: '' };
    }
