...
// Sinónimos de genitalia masculina (incluye pinga)
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  if (soloMuestra) {
    if (tagInferido && !/chup|mamad|oral|lam/i.test(tagInferido)) { elegido = tagInferido; razon = 'usuario solo muestra → tag mirar/ver'; }
    else if (tagModeloNorm && !/chup|mamad|oral|lam/i.test(tagModeloNorm)) { elegido = tagModeloNorm; razon = 'usuario solo muestra → modelo no-oral'; }
    else { elegido = 'usuario_muestra_su_verga'; razon = 'usuario solo muestra → FORZADO'; }
  } else if (pideOral) {
    if (pidePunta) { elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_solo_la_punta_del_pene', false); razon = 'usuario pidió oral (punta)'; }
    else if (pideMitad) { elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_solo_la_mitad_del_pene', false); razon = 'usuario pidió oral (mitad)'; }
    else if (pideTodo) { elegido = normalizarTag(chica, tagInferidoLibre !== 'hablando' ? tagInferidoLibre : 'chupando_todo_el_pene', false); razon = 'usuario pidió oral (todo)'; }
    else {
    ...
    if (tagModeloNorm && tagModeloNorm !== 'hablando') { elegido = tagModeloNorm; razon = 'modelo específico válido'; }
    else if (tagInferido && tagInferido !== 'hablando') { elegido = tagInferido; razon = 'inferido por acción'; }
    else { elegido = tagModeloNorm || tagInferido || 'hablando'; razon = 'fallback'; }
  } else {
  ...
  if (soloMuestra && (/chup|mamad|oral|lam/i.test(elegido) || elegido === 'hablando')) {
    ...
    if (forzado && forzado !== 'hablando') { elegido = forzado; razon += ' + re-forzado'; }
  }
  if (pideOral && !/chup|lam|oral|pene|verga|mam/i.test(elegido)) {
    ...
    if (forzadoOral && forzadoOral !== 'hablando') { elegido = forzadoOral; razon += ' + CORRECCIÓN FINAL oral'; }
  }
...
export async function enviarMensaje(mensajeUsuario) {
  ...
  let system = armarSystemPrompt(personalidad, estado.nombreUsuario, construirContexto(mensajeUsuario), tags, getLore(estado.nombreUsuario));
  ...
  logGroup('Request', { chica: estado.chica, fase: estado.fase, escenaSex, soloNoSex, mensajeUsuario });
  ...
  const partes = bloques.map((b) => {
    ...
    logGroup(`Tag → ${b.chica}`, { tagModelo: parsed.imagen_tag, tagModeloNorm, tagInferido, hayAccion, razon, tagElegido: elegido, tagFinal: media.tag, soloNoSex: ahoraSoloNoSex });
