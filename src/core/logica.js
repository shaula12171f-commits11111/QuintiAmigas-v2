 ...
 * Itera preguntas simples: accion → sujeto → ropa → tag sugerido.
 */
function razonarTagTestingIA(chica, mensajeUsuario, textoBot, tagIA, tagRealUsado) {
  ...
  if (/chup|mam[ao]|oral|lam[ei]/.test(mu + tb)) accion = 'oral';
  else if (/doggy|perrito|a cuatro/.test(mu + tb)) accion = 'doggystyle';
  else if (/misioner/.test(mu + tb)) accion = 'misionero';
  else if (/foll|cog|penetr|meto/.test(mu + tb)) accion = 'penetracion';
  else if (/nalgue|cachetad/.test(mu + tb)) accion = 'nalguear';
  else if (/agarr[oa].*culo|toco.*culo|toc[ao].*culo/.test(mu + tb)) accion = 'usuario_toca_culo';
  else if (/muestra.*culo|culo.*muestra|enseña.*culo/.test(mu + tb)) accion = 'ella_muestra_culo';
  else if (/beso|besarte|besando/.test(mu + tb)) accion = 'beso';
  else if (/desnud/.test(mu + tb)) accion = 'desnudarse';
  ...
  else if (accion.startsWith('oral') || accion === 'penetracion' || accion === 'doggystyle') sujeto = `escena sexual con ${chica}`;
  else sujeto = `interaccion con ${chica}`;
