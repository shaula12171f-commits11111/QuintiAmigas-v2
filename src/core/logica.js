...
const PATRON_SEXO = /\b(foll|chup|mamad|mam[ao]|deepthroat|te la meto|métela|cog[eé]|por el culo|en el culo|follando|penetra|en (tu|la) boca|hasta el fondo|toda la (pija|verga)|handjob|paja|corr[ei]|semen|69|doggy|misioner|cowgirl)\b/i;
...
function esSoloMuestra(mensaje) {
  ...
  const pideActo = /\b(chup|mam[ao]|mamad|en (tu|la) boca|deep|foll|met[eo]|cog|paja|handjob|te la meto|métela)\b/i.test(m);
...
function actualizarFaseSegunUsuario(mensaje) {
  ...
  if (estado.fase !== FASE.INTIMO && /chup|foll|met[eo]|cog|mam[ao]|mamad/i.test(m)) {
    estado.fase = FASE.INTIMO;
  }
}
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  const pidePunta = /solo la punta|chupa.*(punta|cabeza)|cabeza del/i.test(user);
  ...
  if (soloMuestra) {
    ...
    if (pidePunta || pideMitad || pideTodo) {
      ...
      razon = 'nivel oral del USUARIO → inferido';
  ...
  if (soloMuestra && (/chup|mamad|oral/i.test(elegido) || elegido === 'hablando')) {
...
export async function enviarMensaje(mensajeUsuario) {
  ...
  if (estado.fase !== FASE.INTIMO && !soloMuestraUsuario && /chup|foll|met[eo]|cog|mam[ao]/i.test(mensajeUsuario)) {
    estado.fase = FASE.INTIMO;
  }
  ...
  const ahoraEscenaSex = !soloMuestraUsuario && (esEscenaSex() || /chup|foll|met[eo]|cog|mam[ao]/i.test(mensajeUsuario));
