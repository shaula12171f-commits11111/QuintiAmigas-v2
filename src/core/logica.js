...
function resolverIntencionUsuario(mensaje) {
  ...
  if (/agarr[oa].*culo|culo.*agarr|tomo.*culo|manose.*culo|grab.*ass|squeeze.*ass/.test(t)) {
    return { tagHint: ['usuario_agarra_el_culo', 'agarra_el_culo', 'agarrando_culo'], label: 'agarrar_culo' };
  }
  if (/nalgue|cachetad|azote.*culo|pego.*culo/.test(t)) {
    return { tagHint: ['usuario_nalguea_el_culo', 'nalg', 'usuario_nalguea'], label: 'nalguear' };
  }
  return null;
}
...
function elegirTag(chica, tagModelo, textoBloque, textoUsuario, soloNoSex) {
  ...
  if (intencion) {
    const buscarSinFiltro = intencion.label === 'muestra' || intencion.label.startsWith('oral') || intencion.label === 'agarrar_culo' || intencion.label === 'nalguear';
  ...
  // Sin intención clara del usuario: en modo no-sex NO inventar pose a partir del texto del bot
  if (!intencion && soloNoSex) {
    // Solo aceptar tag del modelo si es neutro; si no, hablando
    let elegido = 'hablando';
    let razon = 'sin_intencion_usuario→hablando';
    if (tagModelo && tagModelo !== 'hablando') {
      const neutro = /hablando|selfie|ropa_|besando$|mostrando_sujetador|quitandose|moviendo_el_culo|viendo_verga|usuario_muestra/i.test(String(tagModelo));
      const sexish = esTagSex(tagModelo) || /chup|foll|doggy|anal|cowgirl|mision|handjob|paja|oral|bola|culo_tanga|mostrando_culo/i.test(String(tagModelo));
      if (neutro && !sexish) {
        elegido = normalizarTag(chica, tagModelo, true);
        razon = 'modelo_neutro';
      }
    }
  ...
  if (soloNoSex) {
    ...
    if (!esMuestraTag && (esTagSex(elegido) || /chup|foll|doggy|anal|cowgirl|mision|handjob|paja|oral|bola|mostrando_culo/i.test(String(elegido || '')))) {
      elegido = 'hablando';
      razon += '+forzado_nosex';
    }
  }
