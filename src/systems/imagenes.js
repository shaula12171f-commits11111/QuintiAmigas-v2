...
export function extraerIntencion(textoUsuario) {
  ...
  if (/\bbola(s)?\b/.test(t) || /\btesticul/.test(t)) {
    ...
    if (!intent.acciones.size) intent.acciones.add('chupar');
  }
...
function autoMetaFromTagName(tag) {
  ...
  const meta = {
    accion: [], zona: [], lado: null, intensidad: [], excluye: [], grupo: 'auto', peso: 1.0
  };
...
export function matchTagAvanzado(chica, textoUsuario, textoBot = '', prevTag = null, soloNoSex = false) {
  ...
  // Si el USUARIO no pidió ninguna acción/pose/zona, NO inventar tag sexy
  // a partir del texto del bot (ej. "vamos al parque" no debe dar mostrando_culo_tanga).
  const usuarioSinIntencion =
    intent.acciones.size === 0 &&
    intent.zonas.size === 0 &&
    intent.poses.size === 0 &&
    intent.intensidad.size === 0;

  if (usuarioSinIntencion) {
    return { tag: 'hablando', confianza: 0, razon: 'usuario_sin_intencion→hablando', candidatos: [] };
  }
...
function fallbackPorGrupo(tags, intent, prevTag) {
  ...
  return tags.includes('hablando') ? 'hablando' : (tags[0] || 'hablando');
}
...
export function listarTags(chica) {
  const d = QuintiImagenesPrueba?.[chica];
  if (!d || !d.imagenes) return ['hablando'];
  return Object.keys(d.imagenes);
}

export function getImagenSelector(chica) {
  return QuintiImagenesPrueba?.[chica]?.imagenSelector || '';
}
